...
public class TransaksiPenjualanKaryawan extends javax.swing.JFrame {
    private HashMap<String, Integer> getBahanMenu(String idMenu) throws ClassNotFoundException {
        try{
            Connection conn = (Connection) Koneksi.koneksi.getKoneksi();
            Statement s = conn.createStatement();
            ResultSet r = s.executeQuery("SELECT id_bahan, jumlah FROM detail_produk WHERE id_produk = '"+idMenu+"'");
            // menyimpan data sementara dari bahan
            HashMap<String, Integer> data = new HashMap();
            
            // mendapatkan data bahan-bahan
            while(r.next()){
                data.put(r.getString("id_bahan"), r.getInt("jumlah"));
            }
...
this.setIconImage(icon.getImage());
    ...
    private void initComponents() {
        ...
        jPanel1.add(jButton2, new org.netbeans.lib.awtextra.AbsoluteConstraints(430, 390, 610, 30));
...
//        showData();
      ...
      try{
       ...
       while(rs.next()){
        return Integer.parseInt(rs.getString("Stok_bahan"));
            ...
            e.printStackTrace();
            //karo nyantay ya heeh aku jek nonton anime juga
       }   
        return 0;
   }
   ...
   //ngeleg sek an nb ne
      //  JOptionPane.showMessageDialog(null, "Test Get Data = " + getCurrentStockMenu());
